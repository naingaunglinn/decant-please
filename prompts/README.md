...
3. Run the prompts **in order**, one per session (or one per task in the same session):
   ...
   | 34 | `34-studio-operability.md` | Tenancy follow-up kit C — shop lifecycle states, roles beyond `is_studio`, impersonation audit, registry table, Studio tokens | **PR-1 built (v30): shop lifecycle enum + Filament Shield roles (studio_admin/shop_owner/shop_staff, non-team); 295 tests.** PR-2 (impersonation/audit) + PR-3 (registry redesign, tokens, sidebar) still to come — three PRs, not one (`TENANCY-KIT.md` session 7) |
