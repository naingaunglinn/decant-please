...
| 35–42 | `35-generic-shop-plan.md` | **Generic-shop refactor** (Decant = first template) — parity baseline, product/variant model, code-defined templates + attributes, clothing template, template-driven status labels, stock modes, module toggles, design-spec sync. One umbrella plan, one section + issue per step. | **35 built (v39): parity baseline `GenericShopParityTest` (#104); 364 tests. 36a built (v40): products + product_variants schema, models, admin, archived variants (#105); API contract unchanged; 376 tests. 36b built (v41): `/products` API, checkout by `variant_id`, storefront `/product/[slug]` + redirect (#105); `/fragrances` aliases + legacy payload kept until go-live; 386 tests. 37a built (v42): code-defined templates, `products.attributes` + `search_text`, per-shop `categories`, template-driven admin + `/meta` `filters` (#106); API additive; 401 tests. 37b built (v43): storefront filters, product page and cards render from `/meta` `filters` and `attributes` (+ `show`) (#106); 402 tests. 38a built (v44): clothing template, Size + Color variants with photos in the admin, option filters, studio category pick (#107); API additive; 418 tests on SQLite and Postgres. 38b built (v45): storefront option picker + variant photo, option filters, optional brand (`brand: null`), size guide, demo clothing shop (#107); 428 tests on SQLite and Postgres. 39 built (v46): `decanted` → `prepared`, `decant_date` → `prep_date`, status labels from the shop template through one resolver, tracking `status_labels` + `decant_date` alias (#126); 434 tests on SQLite and Postgres. 40a built (v47): stock modes by template — pooled (`stock_ml` → `stock_amount`) and per variant (`product_variants.stock_qty` + `unit_cost_mmk`), draw-down under row locks in one transaction, per-mode cost snapshot, low-stock panel for both (#128); API unchanged; 445 tests on Postgres 17 (444 + 1 Postgres-only skip on SQLite). 40b built (v48): Myanmar weight units — `products.stock_unit` (ml / kyatthar, viss display only), the reference pair renamed off `_ml`, a frozen `order_items.measure`, weighed variants in the admin, the unit-switch guard (#128); API shape unchanged; 457 tests on Postgres 17. 41 built (v49): module toggles — `shop_settings.modules` (null = template defaults), one resolver `App\Support\Modules`, a Features page, pages/resources/widgets/fields hidden when off, `/meta` `modules`, promo codes refused server-side (#131); 469 tests on Postgres 17. 42 built (v50): design-spec sync, docs only — tenancy design doc §2/§7/§8 (renames, `categories`, templates + modules on `shop_settings`, `shops/{id}/` storage as built, the `withoutTenancy()` ledger: 2 of 5, none added by 35–41), findings name map, `api.md` pass (payment-proof + platform host endpoints, checkout `payment_method` + `proof`, shop + IP rate limits) (#133).** Refactor complete — review stop after step 38 (amended by #115; steps 36–38 amended from the catalog review) |
...
| #134 | issue #134 | **Found in step 42, fixed**: the tracking receipt names an item from its frozen `fragrance_name_snapshot` (not the live product); `decant:fresh-start` deletes variant photos; `POST /orders/payment-proof` refuses (409) a paid, cancelled or rejected order; generic "item"/"product" wording in the two customer-facing "fragrance" errors | **built (v51)**, stacked on #135; 474 tests on Postgres 17 |
| 44 | `44-self-serve-signup.md` | **Self-serve sign-up** — one shop-registration method (Studio + sign-up), reserved slugs, automatic `{slug}.cornerarea.me`, the seller's own Publish; then the sign-up pages, phone verification, Publish button | **44a built (v52): `ShopRegistration::register()` (shop, category, owner, platform address, geography), DNS-label + reserved slug rules, `Shop::publish()` onboarding → live only (#137); no migration; 502 tests on Postgres 17. 44b built (v53): one sign-up page at `/admin/register` (account + phone code + shop, through `register()`), `PhoneVerification` (cache, fails closed; `log` driver local/testing only), `users.phone` + `phone_verified_at`, Publish button + verified-phone rule in `Shop::publish()`; 525 tests on Postgres 17.** The real code-sender driver is queue row 11c (needs-owner) |
| 45 | `45-help-button.md` | **Help button** — admin → studio on Telegram / Viber by deep link (the seller sends, the platform sends nothing); also under the login and sign-up forms; platform env, blank = hidden | **built (v54)** (#140); no migration; 535 tests on Postgres 17 |
| 46 | `46-design-system.md` | **Storefront design system** — a validated design config per shop (base design, four colours, a font, a section list), three presets per category, append-only design history with publish/undo; then the storefront renders it (46b) and a manual editor (46c) | **46a built (v55): `shop_designs` + `shop_settings.published_design_id`, section library (13 sections), `DesignConfig` validator, 6 presets (decant, clothing), `Designs` publish/undo, Design page (out of the menu until 46b) (#142); API unchanged; 606 tests on Postgres 17. 46b built (v56): `/meta` `design` (image paths as URLs, null never 500s), theme as CSS variables on the tenant wrapper, home page from the section list, decant Clean parity proven by `verify-design.mjs`, Design page in the menu (#142); 610 tests on Postgres 17.** 46c is queue row 13c |
