...
3. Run the prompts **in order**, one per session (or one per task in the same session):
   ...
   | 23 | `23-production-schedule-calendar.md` | Month calendar view over the production schedule — domain-layer aggregation, vendored FullCalendar (no plugin, no build-path change) | built |
