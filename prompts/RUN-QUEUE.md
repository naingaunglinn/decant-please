...
| 13c | Step 46c: manual design editor — colours (contrast rule in words), font, section order + on/off, section text, decoration image upload to `shops/{id}/design/`; save = new row, publish = live. Before the colour form opens: validate primary on background (used as link/outline text) and keep derived muted / dark-tile sub-text ≥ 4.5:1 (#144 review) | `prompts/46-design-system.md` §Split (split from row 13); issue #142, don't file another | todo | | |
