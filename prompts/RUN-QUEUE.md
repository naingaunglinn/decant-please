...
| 11b | Step 44b: seller sign-up in the admin panel (account + create-your-shop), phone verification (sender interface, fails closed when unconfigured), Publish button, English + Burmese | §44b (split from row 11); issue #137, don't file another | in-progress | 137-seller-signup | |
| 11c | Phone-code provider driver for sign-up (a `CodeSender` for the owner's chosen SMS / Telegram Gateway account); production sign-up stays off until it exists | §44b "As built"; the provider and its account are the owner's decision | needs-owner | | |
