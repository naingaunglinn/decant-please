...
├── tests/Feature/                          # 361 tests: parity baseline, domain, admin, public API, promo, payments, stock, CSV import, Telegram, invoices, schedule, tenant isolation, storefront hosts, shop lifecycle, Shield roles, impersonation audit, studio registry
...
361 tests / 1,585 assertions on an in-memory SQLite database — 43 of them in
`TenantIsolationTest`, the two-shop isolation suite, 22 in
`StorefrontHostResolutionTest` (host → shop mapping + dynamic CORS, ADR-0004), and 7 in
