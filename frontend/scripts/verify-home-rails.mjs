...
const featuredSlugs = (
  await featuredSection.locator('a[href^="/product/"]').evaluateAll((as) => as.map((a) => a.getAttribute("href")))
).map(slugOf);
...
await page.locator('main a[href^="/product/"]').first().waitFor(); // grid streams in behind loading.tsx
const shopSlugs = (
...
check(
  ...
  (await page.locator(`a[href="/product/${target}"]`).count()) === 1,
  `count=${await page.locator(`a[href="/product/${target}"]`).count()}`,
);
check(
  "recently-viewed rail does not repeat the featured card",
  (await rvSection.locator(`a[href="/product/${target}"]`).count()) === 0,
);
check(
  `rail still shows non-featured "${nonFeatured}"`,
  (await rvSection.locator(`a[href="/product/${nonFeatured}"]`).count()) === 1,
);
